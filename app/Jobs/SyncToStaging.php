...
class SyncToStaging implements ShouldQueue
{
    ...
    {
        $mainRoles = DB::connection('pgsql')->table('roles')->get();
        $staging = DB::connection('pgsql_staging')->table('roles');

        foreach ($mainRoles as $role) {
            $staging->updateOrInsert(
                ['id' => $role->id],
                [
                    'name' => $role->name,
    ...
    private function syncServers(): void
    {
        ...
        $staging = DB::connection('pgsql_staging')->table('servers');
    ...
    private function syncDatabases(): void
    {
        ...
        $staging = DB::connection('pgsql_staging')->table('databases');

        foreach ($mainDbs as $db) {
            $staging->updateOrInsert(
                ['id' => $db->id],
                [
    ...
    {
        $mainMetrics = DB::connection('pgsql')
            ->table('server_metrics')
            ->where('created_at', '>=', now()->subHours(24))
            ->get();
        
        $staging = DB::connection('pgsql_staging')->table('server_metrics');
    ...
    private function syncDbMetrics(): void
    {
        ...
        $staging = DB::connection('pgsql_staging')->table('db_metrics');
    ...
    {
        $mainAlerts = DB::connection('pgsql')
            ->table('alerts')
            ->where('created_at', '>=', now()->subDays(7))
            ->get();
        
        $staging = DB::connection('pgsql_staging')->table('alerts');

        foreach ($mainAlerts as $alert) {
            $staging->updateOrInsert(
                ['id' => $alert->id],
                [

...
<table class="w-full table-auto min-w-[650px]">
                <thead>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="py-2 px-3 text-xs font-medium text-gray-800 truncate">{{ $user->name }}</td>
                        <td class="py-2 px-3 text-xs text-gray-600 truncate max-w-[150px]">{{ $user->email }}</td>
                        <td class="py-2 px-3 text-xs text-gray-600 truncate">{{ $user->organisation->name ?? '-' }}</td>
                        <td class="py-2 px-3 text-center">
                            <button wire:click="toggleActive({{ $user->id }})" class="px-1.5 py-0.5 rounded text-xs font-medium {{ $user->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $user->is_active ? 'Active' : 'Inactive' }}
                            </button>
                        </td>
                        <td class="py-3 px-3 text-right">
                            <div class="flex gap-1 justify-end items-center">
                                <button wire:click="viewUser({{ $user->id }})" class="text-indigo-600 hover:text-indigo-800 p-0.5 rounded-sm hover:bg-indigo-100" title="View">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                    </svg>
                                </button>
                                <button wire:click="openModal({{ $user->id }})" class="text-blue-600 hover:text-blue-800 p-0.5 rounded-sm hover:bg-blue-100" title="Edit">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </button>
                                <button wire:click="confirmDelete({{ $user->id }})" class="text-red-600 hover:text-red-800 p-0.5 rounded-sm hover:bg-red-100" title="Delete">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-4">
<x-tailwind-pagination :links="$users" />
        </div>
    </div>
            ...
            <form wire:submit.prevent="save" wire:loading.attr="disabled">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" wire:model="name" class="w-full px-2 py-1.5 text-sm border rounded-lg focus:ring-1 focus:ring-indigo-500 @error('name') border-red-500 @else border-gray-300 @endif">

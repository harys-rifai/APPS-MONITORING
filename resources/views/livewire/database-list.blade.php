...
<table class="w-full table-auto min-w-[700px]">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($databases as $db)
                            <tr class="hover:bg-gray-50">
                                <td class="py-2 px-3">
                                    <div>
                                        <p class="text-xs font-medium text-gray-800 truncate">{{ $db->name }}</p>
                                        @if($db->server)
                                            <p class="text-xs text-gray-500 truncate">{{ $db->server->name }}</p>
                                        @endif
                                    </div>
                                </td>
                                <td class="py-2 px-3">
                                    <span class="px-1.5 py-0.5 rounded text-xs bg-purple-100 text-purple-700 uppercase">{{ $db->type }}</span>
                                </td>
                                <td class="py-2 px-3 text-xs text-gray-600 truncate">{{ $db->host }}:{{ $db->port }}</td>
                                <td class="py-2 px-3 text-center text-xs font-semibold {{ ($db->realtime_stats['active'] ?? 0) >= $db->active_threshold ? 'text-red-600' : 'text-gray-700' }}">
                                        {{ $db->realtime_stats['active'] ?? 0 }}
                                    </span>
                                </td>
                                <td class="py-2 px-3 text-center text-xs {{ ($db->realtime_stats['idle'] ?? 0) >= $db->idle_threshold ? 'text-yellow-600' : 'text-gray-700' }}">
                                        {{ $db->realtime_stats['idle'] ?? 0 }}
                                    </span>
                                </td>
                                <td class="py-2 px-3 text-center text-xs text-gray-700">{{ $db->realtime_stats['locked'] ?? 0 }}</td>
                                <td class="py-2 px-3 text-center">
                                    <span class="px-1.5 py-0.5 rounded text-xs font-medium {{ ($db->is_reachable ?? false) ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                        {{ ($db->is_reachable ?? false) ? 'Online' : 'Offline' }}
                                    </span>
                                </td>
                                <td class="py-3 text-right">
                                        <div class="flex gap-1 justify-end items-center">
                                            <a href="{{ route('database.monitor', $db->id) }}" class="text-indigo-600 hover:text-indigo-800 p-0.5 rounded-sm hover:bg-indigo-100" title="Monitor">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path>
                                                </svg>
                                            </a>
                                            <button wire:click="openModal({{ $db->id }})" class="text-blue-600 hover:text-blue-800 p-0.5 rounded-sm hover:bg-blue-100" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                </svg>
                                            </button>
                                            <button wire:click="confirmDelete({{ $db->id }})" class="text-red-600 hover:text-red-800 p-0.5 rounded-sm hover:bg-red-100" title="Delete">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mt-4">
                    <x-tailwind-pagination :links="$databases" />
                </div>
            </div>
        </div>
                ...
                <div class="bg-white rounded-xl p-6 w-full max-w-md border border-gray-200 shadow-lg relative z-10">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-lg font-semibold">{{ $databaseId ? 'Edit Database' : 'Add Database' }}</h3>
                        </div>
...
<label class="block text-xs text-gray-600 mb-1">Organisation</label>
                                <select wire:model="organisation_id" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-800 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" @error('organisation_id') border-red-500 @enderror>
                                    <option value="">Select organisation</option>
                                    @error('organisation_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                            ...
                            <div>
                                ...
                                <input type="text" wire:model="name" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-800 focus:outline-none focus:border-indigo-500" @error('name') border-red-500 @enderror>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="text" wire:model="host" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-gray-800 focus:outline-none focus:border-indigo-500" @error('host') border-red-500 @enderror>
                                    @error('host') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                                </div>
                                <div>

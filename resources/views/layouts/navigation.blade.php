<nav x-data="{ open: false }" class="bg-gray-800 border-b border-gray-700">
            ...
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}">
                        <span class="text-xl font-bold text-white">DB Monitor</span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('servers')" :active="request()->routeIs('servers')" class="text-gray-300 hover:text-white">
                        {{ __('Servers') }}
                    </x-nav-link>
                    <x-nav-link :href="route('databases')" :active="request()->routeIs('databases')" class="text-gray-300 hover:text-white">
                        {{ __('Databases') }}
                    </x-nav-link>
                </div>
            </div>
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <x-dropdown-link :href="route('logout')" class="text-gray-300"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('servers')" :active="request()->routeIs('servers')" class="text-gray-300 hover:text-white">
                {{ __('Servers') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('databases')" :active="request()->routeIs('databases')" class="text-gray-300 hover:text-white">
                {{ __('Databases') }}
            </x-responsive-nav-link>
        </div>
